...
class File extends Model2
{
	static public function __schema()
	{
		return [
			...
			'user' => self::field()->depends('user/user')->default(uFrag()->user)->null(),
	...
	static public function uploaded_file($files, $dir = NULL, $file_id = NULL, $var = NULL)
	{
		...
		{
			if (($file = uFrag()->model2('file', $file_id)) && $file->id)
			{
				@unlink($file->path);

				return $file->set('user', uFrag()->user)
