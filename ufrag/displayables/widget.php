...
class Widget extends Displayable
{
	...
	{
		return is_int($widget) ? $this->widget_id($widget) : forward_static_call_array('UF\uFrag\Addons\Widget::__load', [uFrag(), func_get_args()]);
	}
	...
	{
		$widget_data = uFrag()->db->from('nf_widgets')
									->where('widget_id', $this->_widget)
									->row();

		if ($widget_data && ($widget = uFrag()->widget($widget_data['widget'])) && $widget->is_enabled())
		{
			$widget->data = uFrag()->array;
			...
			$style = function($output) use ($widget_data){
				if (is_a($output, 'UF\uFrag\Libraries\Panel'))
				{
