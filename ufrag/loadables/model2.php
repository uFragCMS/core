...
abstract class Model2 extends uFrag implements Loadable
{
	...
	public function __toArray($objects = [])
	{
		...
		foreach ($values as &$value)
		{
			...
			else if (is_a($value, 'UF\uFrag\Loadables\Model2') && !$value())
			{
				$value = NULL;
			}
	...
	public function action($action)
	{
		...
		if ($caller == uFrag() && $this->__name == 'user')
		{
			$caller = uFrag()->module($this->__name);
		}
	...
	public function tracking(&$unread = FALSE)
	{
		...
		if (!array_key_exists($this->id, $this->__tracking) && $this->date->diff(uFrag()->user->registration_date) > 0 && (!array_key_exists(0, $this->__tracking) || $this->date->diff($this->__tracking[0]) > 0))
		{
			$unread = TRUE;
		...
		if (!$this->_data)
		{
			...
			if (($auto_id = $this->_db()->insert($this->_table(), $values)) !== NULL)
			{
				foreach ($this->_schema() as $name => $field)
				{
					...
					{
						$this->_data[$field->i] = uFrag()	->model2('i18n')
	...
	{
		return uFrag()->db(defined('static::DB') ? static::DB : 'default');
	}
