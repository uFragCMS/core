...
class i_0_2 extends uFrag
{
	public function up()
	{
		$this->load = uFrag();

		$this	->network('https://ufrag.download')
				->stream($file = 'cache/monitoring/ufrag.zip');

		if ($zip = zip_open($file))
		{
			while ($zip_entry = zip_read($zip))
			{
				if (preg_match('#^('.implode('|', ['addons/', 'css/', 'fonts/', 'images/', 'js/', 'config/ufrag.php']).')#', $entry_name = zip_entry_name($zip_entry)))
				{
		...
		//Dispositions
		$replacement = [
			'O:3:"Row"'                                => 'O:27:"UF\\uFrag\\Displayables\\Row"',
			'O:3:"Col"'                                => 'O:27:"UF\\uFrag\\Displayables\\Col"',
			'O:12:"Panel_widget"'                      => 'O:30:"UF\\uFrag\\Displayables\\Widget"',
		...
		$update_disposition = function($serialized) use (&$replacement){
			...
			return unserialize($result = "O:27:\"UF\\uFrag\\Libraries\\Array_\":1:{s:9:\"\0*\0_array\";".$result."}") !== FALSE ? $result : $serialized;
		};
		...
		//Copyright
		$this->config('nf_copyright', utf8_htmlentities('Copyright {copyright} {year} {name}, tous droits réservés <div class="pull-right">Propulsé par {ufrag}</div>'));
		$this->db->insert('nf_addon', [
			'type_id' => 3,
			'name'    => 'copyright',
			'data'    => serialize([
