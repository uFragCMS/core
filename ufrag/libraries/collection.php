...
class Collection extends Library
{
	...
	public function __invoke($model = NULL)
	{
		if ($model)
		{
			if (!is_a($model, 'UF\uFrag\Loadables\Model2'))
			{
	...
	public function __call($name, $args)
	{
		if ($name != 'clone' && $this->_db)
		{
			...
			else
			{
				...
				return is_a($result, 'UF\uFrag\uFrag') ? $this : $result;
			}
		}
	...
	public function paginate($page, $limit = 20)
	{
		...
		$this->pagination = uFrag()->pagination($this->_db, $page, $limit);
