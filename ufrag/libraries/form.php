...
class Form extends Library
{
	...
	static private function _token($id)
	{
		...
		if ($tokens === NULL)
		{
			$tokens = uFrag()->session('form') ?: [];
		}

		if (empty($tokens[$id]))
		{
			uFrag()->session->set('form', $id, $tokens[$id] = unique_id(array_merge([$id], $tokens)));
		}
			...
			if ($this->_has_upload())
			{
				...
				foreach ($this->_rules as $var => $options)
				{
					if (isset($options['type']) && $options['type'] == 'file')
					{
						...
						{
							uFrag()->model2('file', $options['value'])->delete();
							$options['value'] = $post[$var] = 0;
						}

						if (!empty($files['tmp_name'][$var]))
						{
							if (!($post[$var] = uFrag()->model2('file')->static_uploaded_file($files, isset($options['upload']) ? $options['upload'] : NULL, isset($options['value']) ? $options['value'] : NULL, $var)->id))
							{
								$this->_errors[$var] = uFrag()->lang('Erreur de transfert');
								return FALSE;
							}
	...
	private function _check_text($post, $var, $options)
	{
		...
		{
			return uFrag()->lang('La valeur sélectionnée n\'est pas valide|Les valeurs sélectionnées ne sont pas valides', count($post[$var]));
		}
		...
		{
			return uFrag()->lang('Veuillez remplir ce champ');
		}
		...
		if ($is_file && !empty($_FILES[$this->token()]['error'][$var]) && $_FILES[$this->token()]['error'][$var] != 4)
		{
			...
			return uFrag()->lang($errors[$_FILES[$this->token()]['error'][$var]]);
		}
	...
	private function _check_email($post, $var, $options)
	{
		...
		{
			return uFrag()->lang('Veuillez entrer une adresse email valide');
		}
	...
	private function _check_url($post, $var, $options)
	{
		...
		{
			return uFrag()->lang('Veuillez entrer une adresse url valide');
		}
	...
	public function display()
	{
		if ($this->_confirm_deletion)
		{
			...
			if ($this->url->ajax())
			{
				return '<div class="modal-header">
							...
							<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">'.uFrag()->lang('Fermer').'</span></button>
						</div>
						<div class="modal-body">
							'.$message.'
						</div>
						<div class="modal-footer">
							<button type="button" class="btn btn-secondary" data-dismiss="modal">'.uFrag()->lang('Annuler').'</button>
							<a class="btn btn-danger delete-confirm" href="'.url($this->url->request).'" data-form-id="'.$this->token().'" onclick="return confirm_deletion(this);">'.uFrag()->lang('Supprimer').'</a>
						</div>';
			}
			else
			{
		...
		{
			uFrag()->js('https://www.google.com/recaptcha/api.js?hl='.$this->config->lang->info()->name.'&_=');
		...
		{
			$output .= '<div class="form-group row"><div class="offset-3 col-9"><em class="text-muted">'.uFrag()->lang('* Toutes les informations marquées d\'une étoile sont requises').'</em></div></div>';
		}
	...
	private function _display_text($var, $options, $post, $type = 'text')
	{
		...
		if (in_array($type, ['date', 'datetime', 'time']))
		{
			...
			uFrag()	->css('bootstrap-datetimepicker.min')
		...
		else if ($type == 'colorpicker')
		{
			...
			uFrag()	->css('bootstrap-colorpicker.min')
		...
		if ($type == 'file')
		{
			...
			$input = '<div style="margin: 7px 0;"><p>'.icon('fas fa-download').' '.uFrag()->lang('Télécharger un fichier').(!empty($options['info']) ? $options['info'] : '').'</p>'.$input.'</div>';
	...
	{
		uFrag()	->css('bootstrap-iconpicker.min')
					...
					->js_load('	$(".btn.iconpicker").iconpicker({
									...
									rows: 5,
									iconset: "fontawesome",
									labelHeader: "'.uFrag()->lang('{0} sur {1} pages').'",
									labelFooter: "<div class=\"float-right\">'.uFrag()->lang('{2} icônes').'</div>",
									searchText: "'.uFrag()->lang('Rechercher...').'",
	...
	private function _display_date($var, $options, $post)
	{
		...
		{
			$options['value'] = timetostr(uFrag()->lang('d/m/Y'), $options['value']);
		}
		else
		{
			$options['value'] = '';
	...
	private function _display_datetime($var, $options, $post)
	{
		if (isset($options['value']) && $options['value'] !== '')
		{
			$options['value'] = timetostr(uFrag()->lang('d/m/Y H:i'), $options['value']);
		}
		else
		{
			$options['value'] = '';
	...
	private function _display_time($var, $options, $post)
	{
		...
		{
			$options['value'] = timetostr(uFrag()->lang('H:i'), $options['value']);
		}
		else
		{
			$options['value'] = '';
