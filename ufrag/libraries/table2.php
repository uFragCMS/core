...
class Table2 extends Library
{
	...
	public function __invoke($data)
	{
		...
		if (is_a($data, 'UF\uFrag\Libraries\Collection'))
		{
			$this->_collection = $data;
		}
		else if (is_a($data, 'UF\uFrag\Loadables\Model2'))
		{
			$this->_collection = $data->collection();
		}
		else if (is_a($data, 'UF\uFrag\Libraries\Array_'))
		{
	...
	public function col()
	{
		...
		if (!$args)
		{
			if (is_a($content, 'UF\uFrag\Libraries\Table_col'))
			{
		...
		else
		{
			...
			$output .= $this->html()
							...
							->exec(function($html){
								...
								{
									$html->content(uFrag()->lang('Aucun résultat trouvé').$this->_filters_reset()->outline()->color('danger btn-sm'));
								}
								else
								{
									$html->content(uFrag()->lang($this->_no_data ?: 'Il n\'y a rien ici pour le moment'));
								}
							});
		}
