...
class Modal extends Library
{
	...
	public function __invoke($title, $icon = '')
	{
		...
		$this->_header = is_a($title, 'UF\uFrag\Libraries\Label') ? $title : $this->label($title, $icon);
	...
	{
		if (!is_a($button, 'UF\uFrag\Libraries\Button'))
		{
	...
	{
		uFrag()->js_load('$(\'#'.$this->id.'\').modal(\'show\');');
		return $this;
	}
	...
	{
		if (!is_a($callback, 'UF\uFrag\Libraries\Form2'))
		{
			$callback = $this	->form2()
