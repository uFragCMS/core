	...
	public function display()
	{
		...
		if ($this->_is_searchable())
		{
			if ($search)
			{
				...
				$this->_data    = $results;
				$this->_no_data = uFrag()->lang('Aucun résultat ne correspond à la recherche');
			}
			else
			{
		...
		if (empty($this->_data))
		{
			$output = '<div class="clearfix"></div>'.($this->_no_data ?: uFrag()->lang('Il n\'y a rien ici pour le moment'));
		}
		else
		{
			if (!$this->_ajax && $this->_is_searchable())
			{
				$search_input = '	<div class="table-search float-left">
										<div class="form-group has-feedback">
											<input class="form-control" data-provide="typeahead" data-items="5" data-source="'.utf8_htmlentities('['.implode(', ', array_unique(array_filter($words))).']').'" type="text"'.(!empty($search) ? ' value="'.$search.'"' : '').' placeholder="'.uFrag()->lang('Rechercher').'" autocomplete="off" />
										</div>
									</div>';
			}
			...
			if ($this->_pagination && !empty($this->output->module()->pagination) && $this->output->module()->pagination->count() > 10)
			{
				$output .= '<div class="form-group float-left">
								<select class="form-control" style="width: auto;" onchange="window.location=\''.url($this->output->module()->pagination->get_url()).'/\'+$(this).find(\'option:selected\').data(\'url\')" autocomplete="off">
									<option value="10"'. ($this->output->module()->pagination->get_items_per_page() == 10  ? ' selected="selected"' : '').' data-url="page/1/10">'.uFrag()->lang('%d résultat|%d résultats', 10, 10).'</option>
									<option value="25"'. ($this->output->module()->pagination->get_items_per_page() == 25  ? ' selected="selected"' : '').' data-url="page/1/25">'.uFrag()->lang('%d résultat|%d résultats', 25, 25).'</option>
									<option value="50"'. ($this->output->module()->pagination->get_items_per_page() == 50  ? ' selected="selected"' : '').' data-url="page/1/50">'.uFrag()->lang('%d résultat|%d résultats', 50, 50).'</option>
									<option value="100"'.($this->output->module()->pagination->get_items_per_page() == 100 ? ' selected="selected"' : '').' data-url="page/1/100">'.uFrag()->lang('%d résultat|%d résultats', 100, 100).'</option>
									<option value="all"'.($this->output->module()->pagination->get_items_per_page() == 0   ? ' selected="selected"' : '').' data-url="all">'.uFrag()->lang('Tout afficher').'</option>
								</select>
							</div>';
			}
			...
			$output .= '<i>'.uFrag()->lang('%d résultat|%d résultats', $count, $count).($count < $count_results ? uFrag()->lang(' sur %d au total', $count_results) : '').'</i>';
