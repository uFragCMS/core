...
class Form2 extends Library
{
		...
		if ($this->_check === NULL)
		{
			...
			if ($this->_check)
			{
				...
				if ($success)
				{
					...
					if ($this->_success)
					{
						if (is_a($this->_values, 'UF\uFrag\Loadables\Model2'))
						{
	...
	public function form(...$args)
	{
		...
		$this->_is_model = is_a($model, 'UF\uFrag\Loadables\Model2');
	...
	{
		$this->_rules[] = is_a($legend, 'UF\uFrag\Libraries\Forms\Legend') ? $legend : $this->form_legend($legend, $icon);
		return $this;
	}

	public function info($info)
	{
		$this->_rules[] = is_a($info, 'UF\uFrag\Libraries\Forms\Content') ? $info : $this->form_info($info);
		return $this;
	}
	...
	protected function _has_submit()
	{
		foreach ($this->_buttons as $button)
		{
			if (is_a($button, 'UF\uFrag\Libraries\Buttons\Submit'))
			{
				return TRUE;
			}
		}
	...
	protected function _exec()
	{
		if ($this->_values)
		{
			foreach ($this->_rules as $rule)
			{
				if ((is_string($rule) || is_object($rule)) && method_exists($rule, 'value'))
				{
					...
					if (is_a($value, 'UF\uFrag\Loadables\Model2') && !is_a($value, 'UF\uFrag\Models\I18n'))
					{
						$value = $value->id;
					}
		...
		{
			if (!$has_upload && is_a($rule, 'UF\uFrag\Libraries\Forms\File'))
			{
				$has_upload = TRUE;
			}
		...
		foreach ($rules as $rule)
		{
			...
			if ((is_string($rule) || is_object($rule)) && method_exists($rule, 'size') && $rule->size())
			{
				...
				if (!is_a($last, 'UF\uFrag\Libraries\Html'))
				{
