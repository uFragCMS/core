...
class Lang extends Library
{
	...
	static protected function _get($caller, $language, $locale, $key)
	{
		...
		if (!isset($locales[$caller_name = get_class($caller)][$key]))
		{
			...
			foreach (uFrag()->config->langs ?: [uFrag()->config->lang] as $lang)
			{
				$result = NULL;
	...
	public function __toString()
	{
		if ($this->_locale === NULL)
		{
			$locale   = $this->_name;
			$args     = $this->_args;
			$language = isset($this->__caller) && is_a($this->__caller, 'UF\uFrag\Loadables\Addon') && !empty($this->__caller->info()->language) ? $this->__caller->info()->language : 'fr';
			...
			if (UFRAG_LOGS_I18N)
			{
				if (!isset($lang))
				{
					$lang = uFrag()->model2('addon')->get('language', $language);
				}
