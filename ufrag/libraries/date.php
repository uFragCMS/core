...
class Date extends Library
{
	...
	{
		if (is_a($datetime, 'UF\uFrag\Libraries\Date'))
		{
			return $datetime;
		}
	...
	public function __toString()
	{
		...
		if ($this->_format == 'Y-m-d')
		{
			$output = uFrag()->lang('Le %s', $this->short_date());

			if ($diff < 0)
			{
				if ($timestamp < strtotime('+2 days midnight'))
				{
					$output = uFrag()->lang('Demain');
				}
				else if ($timestamp < strtotime('+8 days midnight'))
				{
					$output = uFrag()->lang('%s prochain', ucfirst($this->locale('l')));
				}
				else if ($timestamp < strtotime('+22 days midnight'))
				{
					$output = uFrag()->lang('Dans %d jours', floor($diff / 87840 * -1));
				}
			}
			else if ($diff > 0)
			{
				...
				{
					$output = uFrag()->lang('Hier');
				}
				else if ($timestamp >= strtotime('7 days ago midnight'))
				{
					$output = uFrag()->lang('%s dernier', ucfirst($this->locale('l')));
				}
				else if ($timestamp >= strtotime('20 days ago midnight'))
				{
					$output = uFrag()->lang('Il y a %d jours', floor($diff / 87840));
				}
			}
			else
			{
				$output = uFrag()->lang('Aujourd\'hui');
			}
		}
		else
		{
			$output = uFrag()->lang('Le %s à %s', $this->short_date(), $this->short_time());

			if ($diff < 0)
			{
				if ($timestamp < strtotime('+1 days midnight'))
				{
					$output = uFrag()->lang('Aujourd\'hui à %s', $this->short_time());
				}
				else if ($timestamp < strtotime('+2 days midnight'))
				{
					$output = uFrag()->lang('Demain à %s', $this->short_time());
				}
				else if ($timestamp < strtotime('+8 days midnight'))
				{
					$output = uFrag()->lang('%s prochain à %s', ucfirst($this->locale('l')), $this->short_time());
				}
				else if ($timestamp < strtotime('+22 days midnight'))
				{
					$output = uFrag()->lang('Dans %d jours à %s', floor($diff / 87840 * -1), $this->short_time());
				}
			}
			else if ($diff > 0)
			{
				...
				{
					$output = uFrag()->lang('Il y a une seconde|Il y a %d secondes', 1);
				}
				else if ($diff <= strtoseconds('30 seconds'))
				{
					$output = uFrag()->lang('Il y a une seconde|Il y a %d secondes', $diff, $diff);
				}
				else if ($diff < strtoseconds('45 seconds'))
				{
					$output = uFrag()->lang('Il y a une seconde|Il y a %d secondes', 30, 30);
				}
				else if ($diff < strtoseconds('50 seconds'))
				{
					$output = uFrag()->lang('Il y a une seconde|Il y a %d secondes', 45, 45);
				}
				else if ($diff < strtoseconds('55 seconds'))
				{
					$output = uFrag()->lang('Il y a une seconde|Il y a %d secondes', 50, 50);
				}
				else if ($diff < strtoseconds('2 minutes'))
				{
					$output = uFrag()->lang('Il y a environ une minute|Il y a %d minutes', 1);
				}
				else if ($diff <= strtoseconds('59 minutes'))
				{
					$output = uFrag()->lang('Il y a environ une minute|Il y a %d minutes', $diff = floor($diff / 60), $diff);
				}
				else if ($diff < strtoseconds('2 hours'))
				{
					$output = uFrag()->lang('Il y a environ une heure|Il y a %d heures', 1);
				}
				else if ($diff <= strtoseconds('23 hours'))
				{
					$output = uFrag()->lang('Il y a environ une heure|Il y a %d heures', $diff = floor($diff / 3660), $diff);
				}
				else if ($timestamp >= strtotime('yesterday'))
				{
					$output = uFrag()->lang('Hier à %s', $this->short_time());
				}
				else if ($timestamp >= strtotime('6 days ago midnight'))
				{
					$output = uFrag()->lang('%s dernier à %s', ucfirst($this->locale('l')), $this->short_time());
				}
			}
			else
			{
				$output = uFrag()->lang('À l\'instant');
			}
		}
	...
	{
		if (!is_a($date, 'UF\uFrag\Libraries\Date'))
		{
	...
	{
		if (!is_a($start, 'UF\uFrag\Libraries\Date'))
		{
			$start = $this->date($start);
		}

		if (!is_a($end, 'UF\uFrag\Libraries\Date'))
		{
			$end = $this->date($end);
		}
