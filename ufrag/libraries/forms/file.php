...
class File extends Labelable
{
	...
	public function __invoke($name, $upload_dir = '')
	{
		...
		$this->_check[1] = function($post, &$data) use ($upload_dir){
			if (($this->_required && !$this->_value) || !empty($_FILES[$this->_name]['name']))
			{
				...
				{
					$this->_errors[] = uFrag()->lang('Type de fichier non autorisé');
				}
				else if (!empty($_FILES[$this->_name]['tmp_name']))
				{
					...
					{
						$data[$this->_name] = uFrag()->model2('file')->static_uploaded_file($_FILES[$this->_name], $upload_dir, $this->_value ? $this->_value->id : NULL);
						...
						{
							$this->_errors[] = uFrag()->lang('Erreur de transfert');
						}
					}
				}
			}
	...
	{
		if (!is_a($file, 'UF/uFrag/Models/File') && ($file = uFrag()->model2('file', $file)) && !$file())
		{
			$file = NULL;
		}
