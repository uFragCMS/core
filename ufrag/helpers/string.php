...
function strtolink($string, $is_html = FALSE)
{
	...
	return preg_replace_callback('_@((?:&quot;(.+?)&quot;)|([^@\s]+))_', function($match){
		...
		if ($users === NULL)
		{
			foreach (uFrag()->db->select('id', 'username')->from('nf_user')->where('deleted', FALSE)->get() as $user)
			{
		...
		return ($user_id = array_search($username, $users)) !== FALSE ? uFrag()->user->link($user_id, $username, '@') : $match[0];
	}, $string);
}
...
{
	return nl2br(strtolink(uFrag()->bbcode->bbcode2html($string), TRUE));
}
