...
class Driver_Query
{
	...
	public function debug()
	{
		...
		return SqlFormatter::format(SqlFormatter::compress($this->sql)).(!empty($this->bind) ? '<br />'.uFrag()->debug->table($this->bind) : '').(!empty($this->error) ? '<div class="alert alert-danger">'.$this->error.'</div>' : '');
	}
}
