...
class uFrag
{
	...
	public function __path($type, $file = '', &$dir = [], $callback = 'check_file')
	{
		...
		foreach ($paths[get_called_class()]($this, $type, $file) as $path)
		{
			if ($callback($path))
			{
				if ((UFRAG_DEBUG_BAR || UFRAG_LOGS) && isset($this->debug) && !is_a($path, 'UF\uFrag\Libraries\Date'))
				{
		...
		if ($this != ($uFrag = uFrag()))
		{
			return $uFrag->__path($type, $file, $dir, $callback);
		}
	}
	...
	public function ___load($type, $name, $args = [])
	{
		...
		$callback = function(&$path) use ($args){
			if (class_exists($path = class_name('UF\\'.str_replace('/', '\\', $path))))
			{
				$path = uFrag($path, $args);
				return TRUE;
			}
		};
	...
	public function __get($name)
	{
		...
		{
			if (!defined('UFRAG_CORE') && substr($name, 0, 5) == 'core_')
			{
				$type = 'core';
				$name = substr($name, 5);
				$args = [];
			...
			{
				$type = 'libraries';
				$args = [property_exists($this, '__caller') ? $this->__caller : (is_a($this, 'UF\uFrag\Loadables\Addon') ? $this : uFrag())];
			}
			...
			if (!is_null($name) && !($object = @uFrag()->___load($type, $name, $args)))
			{
				...
				$object = @uFrag()->___load($type, implode('\\', $class), $args);
			}
	...
	public function __call($name, $args)
	{
		...
		else if (is_a($class = 'UF\uFrag\Displayables\\'.$name, 'UF\uFrag\Displayable', TRUE))
		{
			$callback = uFrag($class);
		}
		else if (is_a($class = 'UF\uFrag\Addons\\'.$name, 'UF\uFrag\Loadable', TRUE))
		{
			return forward_static_call_array($class.'::__load', [uFrag(), $args]);
		}
		else if (is_a($class = 'UF\uFrag\Loadables\\'.$name, 'UF\uFrag\Loadable', TRUE))
		{
			return forward_static_call_array($class.'::__load', [property_exists($this, '__caller') ? $this->__caller : (is_a($this, 'UF\uFrag\Loadables\Addon') ? $this : uFrag()), $args]);
		}
