...
class Checker extends Module_Checker
{
	...
	public function _session_delete($session_id)
	{
		...
		if (!$this->db->from('sessions')->where('user_id', $this->user->id)->where('id', $session_id)->empty())
		{
			$this->ajax();
	...
	public function _messages_compose($user_id = NULL, $username = NULL)
	{
		...
		else
		{
			if ($user_id && $username)
			{
				if (($user = $this->db->select('username')->from('users')->where('id', $user_id)->where('username <>', NULL)->where('deleted', FALSE)->row()) && $username == url_title($user))
				{
					$username = $user;
				}
				else
