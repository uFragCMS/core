...
class Talks extends Module
{
	...
	public function permissions()
	{
		return [
			'talk' => [
				'get_all' => function(){
					return uFrag()->db->select('talk_id', 'name')->from('nf_talks')->where('talk_id >', 1)->get();
				},
				'check'   => function($talk_id){
					if ($talk_id > 1 && ($talk = uFrag()->db->select('name')->from('nf_talks')->where('talk_id', $talk_id)->row()) !== [])
					{
						return $talk;
					}
				},
