...
class Gallery extends Module
{
	...
	public function permissions()
	{
		return [
			...
			'gallery' => [
				'get_all' => function(){
					return uFrag()->db->select('gallery_id', 'title')->from('nf_gallery_lang')->where('lang', $this->config->lang->info()->name)->get();
				},
				'check'   => function($gallery_id){
					if (($gallery = uFrag()->db->select('title')->from('nf_gallery_lang')->where('gallery_id', $gallery_id)->where('lang', $this->config->lang->info()->name)->row()) !== [])
					{
						return $gallery;
					}
				},
