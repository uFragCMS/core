...
<div class="card mb-4">
	<div class="card-body">
		<h6 class="card-title mb-0"><?php echo ($category['icon_id'] ? '<img src="'.uFrag()->model2('file', $category['icon_id'])->path().'" class="img-icon mr-2" alt="" />' : icon('fas fa-book')).' '.$category['title'] ?></h6>
	</div>
	<?php if ($category['image_id']): ?>
	<img class="img-fluid" src="<?php echo uFrag()->model2('file', $category['image_id'])->path() ?>" alt="" />
	<?php endif ?>
</div>
<?php endif ?>
<?php if ($gallery): ?>
<div class="card-columns">
	...
	<div class="card">
		...
		<a href="<?php echo url('gallery/album/'.$gallerie['gallery_id'].'/'.url_title($gallerie['name'])) ?>"><img class="card-img-top" src="<?php echo uFrag()->model2('file', $gallerie['image_id'])->path() ?>" alt="" /></a>
		<?php endif ?>
		<div class="card-body">
