...
namespace UF\Modules\Gallery\Models;
...
class Gallery extends Model
{
	...
	{
		uFrag()->model2('file', $this->db->select('image_id')->from('nf_gallery')->where('gallery_id', $gallery_id)->row())->delete();
	...
	public function add_image($file_id, $gallery_id, $title, $description = '')
	{
		...
		$this->db->insert('nf_gallery_images', [
			'thumbnail_file_id' => uFrag()->model2('file')->static_add($thumbnail, $title)->id,
			'original_file_id'  => uFrag()->model2('file')->static_add($original, $title)->id,
			'file_id'           => $file_id,
			'gallery_id'        => $gallery_id,
			'title'             => $title,
			'description'       => $description
	...
	public function delete_image($image_id)
	{
		...
		{
			uFrag()->model2('file', $file_id)->delete();
		}
	}
	...
	{
		uFrag()->model2('file', $this->db->select('image_id')->from('nf_gallery_categories')->where('category_id', $category_id)->row())->delete();
