	...
	public function index($gallery)
	{
		$gallery = $this->table()
						->add_columns([
							...
							[
								...
								'content' => function($data){
									return '<a href="'.url('admin/gallery/categories/'.$data['category_id'].'/'.$data['category_name']).'"><img src="'.uFrag()->model2('file', $data['category_icon'])->path().'" class="img-icon" alt="" /> '.$data['category_title'].'</a>';
								},
								'sort'    => function($data){
		...
		$categories = $this	->table()
							->add_columns([
								[
									'content' => function($data){
										return '<img src="'.uFrag()->model2('file', $data['icon_id'])->path().'" class="img-icon" alt="" />';
									},
									'size'    => TRUE
								],
								[
		...
		$gallery_table = $this	->table()
								->add_columns([
									[
										...
										'content' => function($data){
											return '<img style="max-width: 80px;" src="'.uFrag()->model2('file', $data['thumbnail_file_id'])->path().'" alt="" />';
										},
										'align'   => 'left',
										'size'    => TRUE
									],
		...
		return $this->row(
			...
			$this->col(
				$this	->panel()
						...
						->body('<img class="img-fluid" src="'.uFrag()->model2('file', $file_id)->path().'" alt="" />')
						->size('col-12 col-lg-5')
			)
		);
	}
