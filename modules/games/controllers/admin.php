...
namespace UF\Modules\Games\Controllers;
						...
						->add_columns([
							[
								'content' => function($data){
									$output = '<img src="'.uFrag()->model2('file', $data['icon_id'])->path().'" class="img-icon" alt="" /> '.$data['title'];
		...
		$maps = $this	->table()
						->add_columns(array_filter([
							...
							$game_id ? NULL : [
								'title'   => 'Jeu',
								'content' => function($data){
									return ($data['icon_id'] ? '<img src="'.uFrag()->model2('file', $data['icon_id'])->path().'" class="img-icon" alt="" /> ' : '').'<a href="'.url('admin/games/'.$data['game_id'].'/'.$data['name']).'">'.$data['game_title'].'</a>';
								}
							],
							[
								'content' => [
