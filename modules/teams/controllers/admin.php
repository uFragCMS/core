		...
		$teams = $this	->table()
						->add_columns([
							...
							[
								...
								'content' => function($data){
									return '<a href="'.url('teams/'.$data['team_id'].'/'.$data['name']).'"><img src="'.uFrag()->model2('file', $data['icon_id'])->path().'" class="img-icon" alt="" /> '.$data['title'].'</a>';
								}
							],
							[
								'title'   => $this->lang('Jeux'),
								'content' => function($data){
									return '<a href="'.url('admin/games/'.$data['team_id'].'/'.$data['game']).'"><img src="'.uFrag()->model2('file', $data['game_icon'])->path().'" class="img-icon" alt="" /> '.$data['game_title'].'</a>';
								}
							],
							[
	...
	public function _edit($team_id, $name, $title, $image_id, $icon_id, $description, $game_id)
	{
		...
		$this	->table()
				->add_columns([
					[
						'content' => function($data){
							return uFrag()->user->link($data['user_id'], $data['username']);
						}
					],
					[
						'content' => function($data){
