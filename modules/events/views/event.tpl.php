...
<?php if ($image_id): ?>
	<a href="<?php echo $link ?>"><img src="<?php echo uFrag()->model2('file', $image_id)->path() ?>" class="img-fluid" alt="" /></a>
<?php endif ?>
<?php if (!empty($match['opponent']))://Matches ?>
<div class="card-body text-center">
	<div class="row no-gutters align-items-center">
		<div class="text-right col-<?php echo ($icon = uFrag()->model2('file', $match['team']['icon_id'])->path()) ? 4 : 5 ?>">
			<h5 class="m-0">
		...
		<div class="text-right col-1">
			<img src="<?php echo uFrag()->model2('file', $match['opponent']['image_id'])->path() ?>" class="img-fluid" alt="" />
		</div>
		<?php endif ?>
		...
		<?php for ($i = 0; $i < count($rounds); $i++) { ?>
			<div class="card-group mb-2">
				...
				<div class="card p-2 col-3 text-center">
					...
					<a href="#"><?php echo $this->label($rounds[$i]['title'], 'far fa-map')->popover_if($rounds[$i]['image_id'], function($id){ return utf8_htmlentities('<img src="'.uFrag()->model2('file', $id)->path().'" class="img-fluid" alt="" />'); })?></a>
				</div>
				<div class="card text-center justify-content-center">
...
<div class="card-footer">
	...
	<ul class="list-inline m-0">
		...
		<li class="list-inline-item"><?php echo icon('far fa-clock') ?> <?php echo '<span data-toggle="tooltip" title="'.timetostr(uFrag()->lang('l j F Y, H:i'), $date).'">'.timetostr(uFrag()->lang('d/m/Y H:i'), $date).'</span>'.($date_end ? '&nbsp;&nbsp;<span data-toggle="tooltip" title="Durée"><i>'.icon('fas fa-hourglass-end').(ceil((strtotime($date_end) - strtotime($date)) / ( 60 * 60 ))).'h</i></span>' : '') ?></li>
		<?php
