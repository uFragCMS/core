...
class Index extends Controller_Module
{
		...
		foreach ($events as $event)
		{
			...
			if ($this->access('events', 'access_events_type', $event['type_id']))
			{
				$panels->append($this	->panel()
										->heading('<a href="'.url('events/'.$event['event_id'].'/'.url_title($event['title'])).'">'.$event['title'].'</a>'.(!empty($data['match']) ? '<div class="float-right">'.($data['match']['game']['icon_id'] ? '<img src="'.uFrag()->model2('file', $data['match']['game']['icon_id'])->path().'" class="img-icon" alt="" />' : icon('fas fa-gamepad')).' '.$data['match']['game']['title'].'</div>' : ''), $icon)
	...
	public function _event($event_id, $title, $type_id, $date, $date_end, $description, $private_description, $location, $image_id, $published, $type, $mode_id, $webtv, $website, $mode_title)
	{
		...
		return $this->_filters()
					->append($this	->panel()
									->heading('<a href="'.url('events/'.$event_id.'/'.url_title($title)).'">'.$title.'</a>'.(!empty($match) ? '<div class="float-right">'.($match['game']['icon_id'] ? '<img src="'.uFrag()->model2('file', $match['game']['icon_id'])->path().'" class="img-icon" alt="" />' : icon('fas fa-gamepad')).' '.$match['game']['title'].'</div>' : ''), $type == 1 ? 'fas fa-crosshairs' : 'far fa-calendar')
