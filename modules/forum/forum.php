...
class Forum extends Module
{
	...
	public function permissions()
	{
		return [
			'category' => [
				'get_all' => function(){
					return uFrag()->db->select('category_id', 'title')->from('nf_forum_categories')->get();
				},
				'check'   => function($category_id){
					if (($category = uFrag()->db->select('title')->from('nf_forum_categories')->where('category_id', $category_id)->row()) !== [])
					{
						return $category;
					}
				},
